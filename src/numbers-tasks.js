...
function getSumOfNumbers(x1, x2, x3) {
  ...
  const summ = numbers.reduce((sum, num) => sum + Number(num), 0);
  return Number(summ.toFixed(1));
}
...
function getCountOfOddNumbers(number) {
  const resultingNumber = Math.abs(number); // Take the absolute value to handle negative inputs

  const count = Math.floor((resultingNumber + 1) / 2);
  return count;
}
